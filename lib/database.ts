...
// Get unique topics with their total time and session count
...
export async function getTopicSummaries(): Promise<TopicSummary[]> {
    const database = await getDatabase();
    const result = await database.getAllAsync<TopicSummary>(`
        SELECT 
            topic,
            COUNT(*) as sessionCount,
            COALESCE(SUM(duration_seconds), 0) as totalSeconds,
            MAX(start_time) as lastSessionTime
        FROM sessions 
        WHERE end_time IS NOT NULL
        GROUP BY topic
        ORDER BY lastSessionTime DESC
    `);
    return result;
}

// Get recent unique topics for quick continue
export async function getRecentTopics(limit: number = 5): Promise<string[]> {
    const database = await getDatabase();
    const result = await database.getAllAsync<{ topic: string }>(`
...
// Update session topic name
export async function updateSessionTopic(id: number, newTopic: string): Promise<void> {
    const database = await getDatabase();
    await database.runAsync(
